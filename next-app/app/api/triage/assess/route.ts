...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Graceful degradation: live vision assessment unavailable (quota/billing).
    // Return a provisional condition report so the return flow can still proceed.
    console.error("[triage/assess] Gemini unavailable, using fallback report:", error);
    const condition = {
      item: "Inspected item",
      overall_condition: "used_good",
      defects: [],
      packaging_status: "unknown",
      accessories_complete: null,
      angles_reviewed: validation.data.frames.length,
      recommendation: "needs_human_review",
      summary:
        "Live AI assessment is temporarily unavailable, so a provisional 'used – good' grade was applied. A reviewer can confirm the final condition.",
      confidence: 0.5,
    };
    const res = NextResponse.json({ condition, degraded: true }, { status: 200 });
    return isOriginAllowed(origin) ? addCorsHeaders(res, origin!) : res;
  }
}
