...
// Scripted guidance used when the live AI service is unavailable (quota, billing,
// network). Keeps the inspection flowing so the demo never hard-fails.
function scriptedGuidance(data: ChatRequest): string {
  if (data.start) {
    return "Hi! I'm your AI inspection assistant. Hold the item up to the camera so I can see it clearly.";
  }
  const modelTurns = data.history.filter((h) => h.role === "model").length;
  const steps = [
    "Great — now slowly rotate the item so I can see the back.",
    "Thanks. Tilt it to show the top and bottom edges.",
    "If you spot any scratches or damage, bring that area closer to the camera.",
    "Almost done — show me the packaging or any accessories if you have them.",
    'Perfect. When you\'re ready, tap "Assess Condition" to finish the inspection.',
  ];
  return steps[Math.min(modelTurns, steps.length - 1)];
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Graceful degradation: the live AI is unavailable (quota/billing/network).
    // Fall back to scripted guidance so the inspection can continue instead of
    // showing a hard error to the user.
    console.error("[triage/chat] Gemini unavailable, using scripted fallback:", error);
    const res = NextResponse.json(
      { response: scriptedGuidance(validation.data), degraded: true },
      { status: 200 }
    );
    if (isOriginAllowed(origin)) {
      return addCorsHeaders(res, origin!);
