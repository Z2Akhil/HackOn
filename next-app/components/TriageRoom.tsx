...
import { useConditionAssessment, ConditionReport } from "../hooks/useConditionAssessment";
...
interface TriageRoomProps {
  /** Fired when a condition assessment finishes (used by the return flow). */
  onAssessmentComplete?: (report: ConditionReport, frames: string[]) => void;
  /** Render in a constrained container (e.g. inside the return modal). */
  embedded?: boolean;
}

export default function TriageRoom({ onAssessmentComplete, embedded = false }: TriageRoomProps = {}) {
  ...
  // When embedded in the return flow, surface the finished report to the parent.
  const onCompleteRef = useRef(onAssessmentComplete);
  useEffect(() => { onCompleteRef.current = onAssessmentComplete; }, [onAssessmentComplete]);
  const lastAssessedFramesRef = useRef<string[]>([]);
  const reportFiredRef = useRef(false);
  useEffect(() => {
    if (assessment.report && !reportFiredRef.current) {
      reportFiredRef.current = true;
      onCompleteRef.current?.(assessment.report, lastAssessedFramesRef.current);
    } else if (!assessment.report) {
  ...
  const handleAssess = useCallback(async () => {
    ...
    lastAssessedFramesRef.current = selected;
    await assessment.assess(selected, triageChat.messages);
  }, [captureFrame, assessment, triageChat.messages]);

  return (
    <div className={embedded ? "triage-room triage-room--embedded" : "triage-room"}>
