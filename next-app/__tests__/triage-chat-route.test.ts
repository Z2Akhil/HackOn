...
describe("POST /api/triage/chat - validation", () => {
  ...
  it("falls back gracefully when GEMINI_API_KEY is not set", async () => {
    ...
    try {
      ...
      const res = await POST(req);
      // Graceful degradation: scripted guidance is returned instead of an error.
  ...
  it("returns a scripted greeting for a start request when API key is missing", async () => {
    const originalKey = process.env.GEMINI_API_KEY;
    delete process.env.GEMINI_API_KEY;
    try {
      const req = createRequest({
  ...
  it("passes validation for message at exactly 2000 characters", async () => {
    ...
    try {
      ...
      const res = await POST(req);
      // Valid request passes validation; Gemini unavailable → graceful 200 fallback.
      expect(res.status).toBe(200);
      const json = await res.json();
      expect(json.degraded).toBe(true);
    } finally {
